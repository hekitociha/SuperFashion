...
1. Доступность (Availability):
    ...
    Пример: Система должна быть доступна для использования не менее чем на 95% времени в течение рабочих дней.

2. Целостность (Integrity):

    Уровень защиты от потери информации и сохранение корректности данных в системе.
    Пример: После резервного копирования данных, система должна проверить их целостность и сообщить об отличиях, если они есть.

3. Совместимость (Interoperability):

    Готовность системы к обмену данными с другими программными системами и интеграции с внешними устройствами.
    Пример: SuperFashion должен корректно работать с платежной системой через телеграмм.

4. Производительность (Performance):

    Скорость реакции системы на запросы и действия пользователей.
    Пример: Время загрузки бота SuperFashion не должно превышать 3 секунды при скорости интернет-соединения 30 Мбит/с.

5. Надежность (Reliability):

    Вероятность работы магазина без сбоев в течение определенного периода времени.
    Пример: Вероятность сбоя системы SuperFashion не должна превышать 0.5% за месяц.

6. Устойчивость (Robustness):

    Способность системы восстанавливаться после сбоев и сохранять работоспособность.
    Пример: При возникновении ошибок при оформлении заказа, система SuperFashion должна сохранить все данные о заказе пользователя.

7. Требования к защите (Safety):

    Системные меры, направленные на предотвращение вреда для пользователей и их данных.
    Пример: SuperFashion должен предоставлять возможность просмотра состава товаров, выделяя особо те компоненты, которые могут вызывать аллергические реакции.
    Пример: Все данные, передаваемые при обработке платежей, должны быть зашифрованы с использованием алгоритма шифрования SHA256 для предотвращения несанкционированного доступа.
...
1. Эффективность (Efficiency):
    ...
    Пример: SuperFashion должен минимизировать время обработки запросов покупателей до 500мс для оптимизации использования серверных ресурсов.

2. Возможность модификации (Modifiability):

    Уровень простоты внесения изменений в дизайн и код магазина.
    Пример: Добавление новых категорий товаров в магазин SuperFashion должно быть простым и понятным процессом для разработчиков.

3. Возможность повторного использования (Reusability):

    Способность программных компонентов магазина быть использованными повторно в других приложениях.
    Пример: Компоненты корзины и оформления заказа SuperFashion должны быть легко переносимы для использования в других интернет-магазинах.
...
4. Масштабируемость (Scalability):
    ...
    Пример: SuperFashion должен масштабироваться в соответствии с увеличением числа посетителей без задержек или сбоев.

5. Проверяемость (Verifiability):

    Легкость проверки программных компонентов на соответствие функциональным требованиям.
    Пример: SuperFashion должен обеспечивать доступ к логам и отчетам о работе системы для проведения тестирования функциональности.
